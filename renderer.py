...
from character import SpidermanDesign

class Renderer:
    def __init__(self, physics, animation):
        self.physics = physics
        self.animation = animation
        self.character_design = SpidermanDesign(Config.RENDER_SIZE)
    ...
    def draw(self, painter, offset_x=0, offset_y=0):
        ...
        # Draw Procedural Spiderman from character.py
        self.character_design.draw(painter, x, y, self.animation)
    ...
    def _draw_debug(self, painter, offset_x, offset_y):
        ...
        painter.setPen(pen)
        painter.drawRect(int(self.physics.x - offset_x), int(self.physics.y - offset_y), Config.RENDER_SIZE, Config.RENDER_SIZE)
        painter.setFont(QFont("Consolas", 10))
        painter.drawText(int(self.physics.x - offset_x), int(self.physics.y - offset_y) - 5, self.animation.current_state)
